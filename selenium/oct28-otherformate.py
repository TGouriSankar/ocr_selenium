...
# Function to process CAPTCHA with OCR
...
try:
    ...
    captcha_field_click = driver.find_element(By.XPATH, '/html/body/div[4]/div/div/div/div/form/div[2]/div[4]/table/tbody/tr[6]/td/div/input')
    captcha_field_click.click()
    random_delay()
...
# Keep trying to solve and submit the CAPTCHA until successful
attempts = 0
while attempts < 5:  # Limit the number of attempts
    attempts += 1
    print(f"Attempt {attempts} to solve CAPTCHA")
    
    # Get the CAPTCHA code using OCR
    captcha_code = get_captcha_code()
    print("CAPTCHA code detected:", captcha_code)

    # Enter the CAPTCHA code
    captcha_field = driver.find_element(By.XPATH, '/html/body/div[4]/div/div/div/div/form/div[2]/div[4]/table/tbody/tr[6]/td/div/input')
    captcha_field.clear()
    random_delay()
    ...
    try:
        ...
        random_delay()
        submit_button.click()
        print("Form submitted with CAPTCHA code:", captcha_code)
        break  # Exit loop if submitted successfully
    except Exception as e:
        print("Submission failed, retrying CAPTCHA:", str(e))
